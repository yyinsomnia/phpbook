...
PHP在内核中是通过zval这个结构体来存储变量的，它的定义在Zend/zend_types.h文件里，简短精炼，只有三个成员组成：
...
保存变量值的value则是zvalue_value类型(PHP7)，它是一个union，同样定义在了Zend/zend_types.h文件里：
...
{
	if (foo->u1.v.type == IS_NULL)　//foo->type == IS_NULL in php5
	{
...
PHP内核以后可能会修改变量的实现方式(果然就改了吧ｏ（╯□╰）ｏ)，所以检测type的方法可能在以后就不能用了。
...
此外获取变量类型的宏还有一个，Z_TYPE，参数是zval型。//喵那个咪的５里面的Z_TYPE_PP在７里面找不到了
...
以上两个宏的定义也在Zend/zend_types.h里，定义分别是：
